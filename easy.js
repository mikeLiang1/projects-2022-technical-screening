...
const altNumbers = (numArray) => {
  // if in wrong place, find next opposite sign and switch their places
  ...
  let sign = 1; // sign 1 means nrgative first
  if (positivearr.length >= negativearr.length) {
    // more positive means first should be positive
    sign = 0;
  }
  for (let i = 0; i < numArray.length; i++) {
    if (sign == 1) {
      // if should be negative, we dequeue negative and make sign opposite
      array.push(negativearr.shift());
      sign = 0;
    } else {
      array.push(positivearr.shift());
      sign = 1;
    }
  }
